...
class _GuidePainter extends CustomPainter {
  ...
  Path _topPath(Size s) {
    ...
    // 왼쪽 넥라인
    path.quadraticBezierTo(cx - neckR * 2, top, cx - shoulderW, top + sleeveH * 0.3);
  ...
  Path _bottomPath(Size s) {
    ...
    // 오른쪽 엉덩이
    path.quadraticBezierTo(cx + hipW + s.width * 0.02, crotch * 0.6, cx + hipW, crotch);
    // 오른쪽 다리 아래
    ...
    // 왼쪽 엉덩이
    path.quadraticBezierTo(cx - hipW - s.width * 0.02, crotch * 0.6, cx - waistW, top);
  ...
  Path _dressPath(Size s) {
    ...
    path.quadraticBezierTo(cx - neckR * 2, top, cx, top + neckR * 0.5);
    path.quadraticBezierTo(cx + neckR * 2, top, cx + shoulderW, top + s.height * 0.04);
    path.lineTo(cx + waistW, waistY);
    path.quadraticBezierTo(cx + hemW * 0.7, waistY + s.height * 0.1, cx + hemW, bottom);
    path.lineTo(cx - hemW, bottom);
    path.quadraticBezierTo(cx - hemW * 0.7, waistY + s.height * 0.1, cx - waistW, waistY);
    path.close();
    return path;
  }
  ...
  Path _rectPath(Size s) {
    ...
    return Path()
      ..addRRect(RRect.fromLTRBR(l, t, s.width - l, s.height * 0.80,
